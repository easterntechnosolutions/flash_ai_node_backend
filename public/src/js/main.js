...
import { addDotBtnsAndClickHandlers } from "../js/embla-nav";
import { listen } from "../js/utils";
import { enter, leave } from "../js/transition";
import SwapAnimation from "../js/animations/SwapAnimation";
import FlowAnimation from "../js/animations/FlowAnimation";
...
async function selectStatisticsTimeframe(_, eventTrigger) {
  ...
  const currentView = document.querySelector(
    `.statistics-view[data-value="${currentButton.dataset.value}"]`
  );

  const newButton = eventTrigger;
  const newView = document.querySelector(
    `.statistics-view[data-value="${newButton.dataset.value}"]`
  );
...
listen(
  "click",
  '.statistics-timeframe[data-value="all"]',
  selectStatisticsTimeframe
);
listen(
  "click",
  '.statistics-timeframe[data-value="today"]',
  selectStatisticsTimeframe
);
listen(
  "click",
  '.statistics-timeframe[data-value="yesterday"]',
  selectStatisticsTimeframe
);
listen(
  "click",
  '.statistics-timeframe[data-value="week"]',
  selectStatisticsTimeframe
);
listen(
  "click",
  '.statistics-timeframe[data-value="month"]',
  selectStatisticsTimeframe
);

const exchangeRatesSection = document.getElementById("exchange-rates-section");
const exchangeRatesContainer = document.getElementById(
  "exchange-rates-container"
);
const exchangeRatesAnimation = new SwapAnimation(exchangeRatesContainer, 1200);
exchangeRatesSection.addEventListener("pointerenter", () => {
  exchangeRatesAnimation.run();
});
exchangeRatesSection.addEventListener("pointerleave", () => {
  exchangeRatesAnimation.stop();
});
...
transfersSection.addEventListener("pointerenter", () => {
  transfersAnimation.run();
});
transfersSection.addEventListener("pointerleave", () => {
  transfersAnimation.stop();
});

const anonPaymentsSection = document.getElementById("anon-payments-section");
const anonPaymentsContainer = document.getElementById(
  "anon-payments-container"
);
const anonPaymentsAnimation = new FlowAnimation(anonPaymentsContainer, 2000);
anonPaymentsSection.addEventListener("pointerenter", () => {
  anonPaymentsAnimation.run();
});
anonPaymentsSection.addEventListener("pointerleave", () => {
  anonPaymentsAnimation.stop();
});
